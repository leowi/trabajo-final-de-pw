...
<nav class="navbar navbar-expand-lg navbar-light bg-white">
  ...
  <ul class="navbar-nav">
    ...
    <ul class="navbar-nav ml-auto">
      <li class="nav-item">
        <a href="/users"><button class="button button2">Users</button></a>
      </li>
      <li class="nav-item">
...
<div class="row">
    <div class="col-sm-12 bg-dark">
      <center class="cent1" >Lista de Usuarios</center>
    </div>
  </div><br>
		<div class="container">
		<input class="form-control" id="myInput" type="text" placeholder="Nombre del usuario..."><br>
			<% if (users.size() > 0) { %>
			    ...
			    <tbody id="myList">					
					...
					<% if(Role.exist(u.getIdRole()).equals("null")) {
						UserJava a = pm.getObjectById(UserJava.class,u.getId());
					...
					<td><%=u.isStatus()%></td>
					<td>
					<a href="/users/delete?id=<%=u.getId()%>" class="btn btn-outline-danger btn-sm RbtnMargin float-right">Delete</button></a>
			    	<a href="/users/view?id=<%=u.getId()%>" class="btn btn-outline-secondary btn-sm RbtnMargin float-right">View</button></a>
			    	<a href="/users/update?id=<%=u.getId()%>" class="btn btn-outline-info btn-sm RbtnMargin float-right">Edit</button></a>
			    	</td>
			    	</tr>
			    	<% } %>
					<% } %>
      ...
      <p><a href="/users/add"> crear nuevo</a></p>
    <% } %>
<script>
