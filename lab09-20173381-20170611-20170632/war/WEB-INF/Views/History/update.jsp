...
<div class="row">
    <div class="col-sm-12 bg-dark">
      <center class="cent1" >Editar Historia</center>
    </div>
  </div><br>
	<div class="container">
	<br><br>
		<form method="get" action="/history/update">
		<input type="hidden" name="id" value="<%=id %>">
			<table class="table">
				<tbody>
			      <tr>
			        <td><label>Nombre del Paciente</label><input type="text" name="name" class="form-control" value="<%= u.getName() %>"></td>
			        <td><label>Fecha de Nacimiento</label><input type="date" name="birthdate" class="form-control" value="<%= u.getBirthdate() %>"></td>
			        <td><label>Genero</label><select name="sex" id="mySelect" class="form-control">
			        	<option value="hombre">Hombre</option>
						<option value="mujer">Mujer</option>
						</select>
						<script>
    						document.getElementById("mySelect").value = "<%= u.getSex() %>";
						</script>
					</td>
			      </tr>
			      <tr>
			        <td><label>Nombre del Profecional</label><input type="text" name="namepro" class="form-control" value="<%= u.getNamepro() %>"></td>
			        <td><label>Fecha de Consulta</label><input type="date" name="date" class="form-control" value="<%= u.getDate() %>" ></td>
			        <td></td>
			      </tr>
	    		</tbody>
    		</table>
    		...
    		<table class="table">
    			<tbody>
    				<tr>
    					<td><label>MOTIVO DE CONSULTA</label><textarea name="rfc" class="form-control" rows="5" cols="10" ><%= u.getRfc() %></textarea></td>
    					<td><label>ANTECEDENTES</label><br>
							<label>Antecedentes Familiares</label><input type="text" name="fh" class="form-control" value="<%= u.getFh() %>" >
							<label>Antecedentes Personales</label><input type="text" name="ph" class="form-control" value="<%= u.getPh() %>" ></td>
    				</tr>	
    				<tr>
    					<td><label>DIAGNÓSTICO</label><br>
							<label>Diagnóstico Presuntivo</label><input type="text" name="pd" class="form-control" value="<%= u.getPd() %>" >
							<label>Diagnóstico Definitivo</label><input type="text" name="dd" class="form-control" value="<%= u.getDd() %>" ></td>
						<td><label>PLAN DE TRABAJO</label><textarea name="wp" class="form-control" rows="5" cols="10" ><%= u.getWp() %></textarea></td>
    				</tr>
    				<tr>
    					<td><label>PRONÓSTICO</label><input type="text" name="fore" class="form-control" value="<%= u.getFore() %>" ></td>
    					<td><label>TRATAMIENTO/RECOMENDACIONES</label><input type="text" name="recommend" class="form-control" value="<%= u.getRecommend() %>" ></td>
    				</tr>
    			</tbody>
    		</table>
    		<center>
				...
				<label><h3>Odontograma</h3></label><br><br>
					<tr>
						...
						<td>
							<label>Especificaciones</label><textarea name="spe" class="form-control" rows="5" cols="35" ><%= u.getSpe() %></textarea><br><br>
							<label>Observaciones</label><textarea name="obs" class="form-control" rows="5" cols="35" ><%= u.getObs() %></textarea>
						</td>
					</tr>
			  </table>
			  </center><br><br>
