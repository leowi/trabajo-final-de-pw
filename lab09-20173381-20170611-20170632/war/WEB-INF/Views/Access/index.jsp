...
<nav class="navbar navbar-expand-lg navbar-light bg-white">
  ...
  <ul class="navbar-nav">
      <li class="nav-item">
        <a href="/access/add"><button class="button button1">Create</button></a>
      </li>      
    </ul>
    <ul class="navbar-nav ml-auto">
      <li class="nav-item">
      ...
      <li class="nav-item">
        <a href="/access"><button class="button button2">Access</button></a>
      </li>
      <li class="nav-item">
...
<div class="row">
    <div class="col-sm-12 bg-dark">
      <center class="cent1" >Lista de Accesos</center>
    </div>
  </div><br>
  <div class="container">
  <input class="form-control" id="myInput" type="text" placeholder="Buscar...">
    ...
    <thead>
		...
		<tbody id="myList">
			...
			<% if(Role.exist(u.getIdRole()).equals("null") || Resource.exist(u.getIdResource()).equals("null")) {
				Access a = pm.getObjectById(Access.class,u.getId());
				pm.deletePersistent(a);
			}%>
			<% if(!(Role.exist(u.getIdRole()).equals("null")) && !(Resource.exist(u.getIdResource()).equals("null"))) { %>
		<tr>
			<td><%=Role.exist(u.getIdRole()) %></td>
			<td><%=Resource.exist(u.getIdResource()) %></td>
			<td <%if(u.isStatus()==false){ %> id="alerta"<%}%>><%=u.isStatus() %></td>
			<td>
				<a href="/access/delete?id=<%=u.getId()%>" class="btn btn-outline-danger btn-sm RbtnMargin float-right">Delete</button></a>
    			<a href="/access/view?id=<%=u.getId()%>" class="btn btn-outline-secondary btn-sm RbtnMargin float-right">View</button></a>
	    		<a href="/access/update?id=<%=u.getId()%>" class="btn btn-outline-info btn-sm RbtnMargin float-right">Edit</button></a></td>
	    </tr>
	   		 <%} %>
	<% } %>
    </tbody>
  ...
  <% } else { %>
      <a>No hay ninguna cuenta.</a>
      <p><a href="../"> volver al inicio</a></p>
      <p><a href="/access/add"> crear nuevo</a></p>
    <% } %>
