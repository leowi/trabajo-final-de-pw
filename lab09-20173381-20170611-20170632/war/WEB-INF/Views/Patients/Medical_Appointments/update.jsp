...
<%Medical_Appointment m_a = (Medical_Appointment)request.getAttribute("medical_appointment"); %>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Actualizar Cita Medica</title>
<link rel="stylesheet"
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-sm-12 bg-dark">
				<center class="cent1">Actualizar Cita Medica</center>
			</div>
		</div>
		<br>
		<div class="container">
		...
		<form method="post" action="/medical_appointments/update">
				<table class="table">
				<input type="text" name="id" value="<%=m_a.getId()%>"
								hidden> 
					<tbody>
						<tr>
						...
						<td><label>Nombre del paciente: </label> <input
								value="<%=p.getName() %>" 
						...
						<%} 
							...
							<td><label>Motivo de la cita medica: </label> <input
								type="text" name="reason" value="<%=m_a.getReason() %>" 
								class="form-control" required placeholder="motivo de la cita medica" ></td>
							<td><label>Fecha de la Cita Medica:
							</label> <input type="date" value="<%=m_a.getAppointment_date() %>" name="appointment_date" 
								class="form-control" required></td>
							
							<td><label>Estado de la cita: </label> <select name="status"
								class="form-control">
									<%
										if (m_a.isStatus() == true) {
									%>
									<option selected value="true">Activa</option>
									<option value="false">Inactiva</option>
									<%
										} else {
									%>
									<option selected value="false">Inactiva</option>
